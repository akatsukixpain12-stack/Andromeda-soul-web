...
`,
  ...
  app.get('/api/auth/google/start', (req: Request, res: Response) => {
    ...
    if (!clientId || !process.env.GOOGLE_CLIENT_SECRET) return res.status(503).send('Google OAuth is not configured. Set GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET.');
    const statePayload = `${Date.now()}.${crypto.randomBytes(24).toString('hex')}`;
    const state = `${statePayload}.${crypto.createHmac('sha256', process.env.GOOGLE_CLIENT_SECRET || 'andromeda-oauth').update(statePayload).digest('hex')}`;
  ...
  app.get('/api/auth/google/callback', async (req: Request, res: Response) => {
    ...
    if (!code || !state) return res.status(400).send('Missing Google OAuth callback parameters.');
    const stateParts = state.split('.');
    const statePayload = stateParts.slice(0, 2).join('.');
    const expectedState = crypto.createHmac('sha256', process.env.GOOGLE_CLIENT_SECRET || '').update(statePayload).digest('hex');
    const suppliedState = Buffer.from(stateParts[2] || '');
    const expectedStateBuffer = Buffer.from(expectedState);
    if (stateParts.length !== 3 || suppliedState.length !== expectedStateBuffer.length || !crypto.timingSafeEqual(suppliedState, expectedStateBuffer) || !Number.isFinite(Number(stateParts[0])) || Date.now() - Number(stateParts[0]) > 600000) return res.status(400).send('Invalid or expired OAuth state.');
    try {
      const redirectUri = process.env.GOOGLE_REDIRECT_URI || `${process.env.APP_URL || 'http://localhost:3000'}/api/auth/google/callback`;
      ...
      const profile = { id: `google:${googleUser.sub}`, name: googleUser.name || googleUser.email, email: googleUser.email, avatar: googleUser.picture || '', provider: 'google', connectedAt: Date.now() };
      res.type('html').send(`<script>localStorage.setItem('andromeda_user_profile_v3',${JSON.stringify(JSON.stringify(profile))});location.replace('/');</script>`);
    } catch (err: any) { res.status(502).send(`Google sign-in failed: ${err.message}`); }
  });
